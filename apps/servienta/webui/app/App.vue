...
const GUIDE_ORDER = ["README", "quickstart", "api", "receivers", "response-control", "faults", "licensing", "console", "configuration", "extending"];
...
async function refresh() {
  error.value = null;
  try {
    version.value = (await api<{ contract: string }>("/v1/version")).contract;
    endpoints.value = await api<Record<string, string>>("/v1/endpoints");
    license.value = await api("/v1/license");
    engineOk.value = true;
...
async function loadReceived() { error.value = null;
  try { const q = run.value ? `?run=${encodeURIComponent(run.value)}` : "";
    received.value = await api<Record<string, unknown>[]>(`/v1/received/${service.value}${q}`); }
  catch (e) { error.value = (e as Error).message; } }
async function reset() { resetMsg.value = null;
  try { await api("/v1/reset", { method: "POST" }); resetMsg.value = "reset to a known state"; received.value = null; }
  catch (e) { error.value = (e as Error).message; } }
async function applyLicense() { licenseMsg.value = null; uploading.value = true;
  try { const parsed = JSON.parse(licenseText.value);
    await api("/v1/license", { method: "PUT", body: JSON.stringify(parsed) });
    licenseMsg.value = "applied — restarting…"; licenseText.value = ""; await waitForEngine();
...
async function removeLicense() { if (!confirm("Remove the license and revert to free mode?")) return;
  ...
  try { await api("/v1/license", { method: "DELETE" }); licenseMsg.value = "removed — restarting…"; await waitForEngine(); }
  catch (e) { licenseMsg.value = (e as Error).message; } finally { uploading.value = false; } }
      ...
      <div style="max-width:1200px;margin:0 auto;padding:0 40px;height:58px;display:flex;align-items:center;gap:14px">
        ...
        <div style="margin-left:auto;display:flex;align-items:center;gap:20px">
          ...
          <span class="mono" style="display:inline-flex;align-items:center;gap:8px;font-size:11px;letter-spacing:0.06em;text-transform:uppercase;border-left:1px solid var(--rule);padding-left:20px"
            ...
            {{ engineOk === null ? "…" : engineOk ? `stand up · contract v${version}` : "stand unreachable" }}
          </span>
        </div>
      </div>
    </header>
    ...
    <main style="max-width:1200px;margin:0 auto;padding:0 40px">
      <div style="border-left:1px solid var(--rule);border-right:1px solid var(--rule);min-height:calc(100vh - 58px);padding:40px 48px 80px">
        <div v-if="view === 'start'">
          ...
          <p style="margin:14px 0 0;max-width:660px;font-size:15px;line-height:1.6;color:var(--ink2)">This console manages a running Servienta stand over its versioned HTTP contract. You can drive it from the tabs above, or directly with <span class="mono" style="font-size:13px">curl</span> from your test suite — the calls below are exactly what this console makes.</p>

          <div style="margin-top:32px">
            <div class="flabel" style="padding-bottom:8px;border-bottom:2px solid var(--ink)">1 · Reach the stand</div>
            <p style="margin:12px 0 0;font-size:14px;line-height:1.6;color:var(--ink2)">Everything is one container: this console on <span class="mono">:5000</span>, the contract on <span class="mono">:5001</span>, the HTTP file server on <span class="mono">:8080</span>, every other service on its own port — see the Stand tab. One command brings it all up:</p>
...
docker run --rm -p 5000:5000 -p 5001:5001 -p 8080:8080 -p 9000:9000 \
  -v "$PWD/fixtures:/fixtures:ro" ghcr.io/servienta/servienta:latest</pre>
            <p style="margin:12px 0 0;font-size:14px;line-height:1.6;color:var(--ink2)">Map more ports the same 1:1 way as you license more services. First check:</p>
            <pre class="mono" style="margin:12px 0 0;font-size:12px;line-height:2;color:var(--ink);overflow-x:auto">curl localhost:5001/api/v1/endpoints</pre>
          </div>
...
curl localhost:5001/api/v1/endpoints
...
curl localhost:5001/api/v1/received/reference

<span style="color:var(--ink3)"># steer a reply (licensed services), inject a fault, reset</span>
curl -X PUT  localhost:5001/api/v1/responses/dns   -d '{"outcome":"nxdomain"}'
curl -X PUT  localhost:5001/api/v1/faults/receivers/syslog -d '{"mode":"drop"}'
curl -X POST localhost:5001/api/v1/reset</pre>
          </div>

          <div style="margin-top:36px">
            <div class="flabel" style="padding-bottom:8px;border-bottom:2px solid var(--ink)">3 · Run the walkthrough</div>
            <p style="margin:12px 0 0;font-size:14px;line-height:1.6;color:var(--ink2)">A one-file script that runs every step above against your stand and prints the result in your terminal. Copy it, save as <span class="mono" style="font-size:13px">walkthrough.sh</span>, and run:</p>
        ...
        <section v-if="license">
          ...
          <div style="margin-top:28px;display:grid;grid-template-columns:200px minmax(0,1fr);gap:32px">
            <div>
              ...
              <p style="margin:8px 0 0;font-size:14px;line-height:1.6;color:var(--ink2)">Paste the license file as issued. Servienta restarts in place to apply it.</p>
            </div>
        ...
        <section style="margin-top:52px">
          ...
          <p style="margin:14px 0 0;max-width:660px;font-size:14px;line-height:1.6;color:var(--ink2)">Every service listens on its own port: this console is <span class="mono" style="font-size:13px">ui</span>, the contract is <span class="mono" style="font-size:13px">control</span>. Addresses are in-container; they hold on your host for every port you mapped 1:1 in <span class="mono" style="font-size:13px">docker run</span>. (DNS sits on 15353 out of the box so 1:1 works on desktops, where mDNS owns 5353/udp.)</p>
