...
Base path `/api/v1`, served on the control port (default `:5001`; the console port `:5000` answers the same paths). The machine-
readable schema ships with the delivery as `apps/servienta/openapi.yaml`; a client
...
```json
{ "control": "[::]:5001", "ui": "[::]:5000", "files-http": "[::]:8080", "reference": "[::]:9000" }
```
...
```
`mode` is `free` or `licensed`; `error` appears when a stored license was
rejected (Servienta then runs free).
...
### `PUT /api/v1/license`
Store an issued license (the request body, as issued) and restart in place to
apply it. The signature is verified at that startup — the endpoint grants no
signing power. `DELETE /api/v1/license` removes it and reverts to free mode.
Returns `202`; Servienta is briefly unavailable while it restarts.

### `POST /api/v1/reload`
Restart Servienta in place to re-read the stored license (the console's
Apply button uses `PUT /api/v1/license`). Servienta still validates the license only at
...
| PUT | `/api/v1/license` | Store a license and restart to apply it |
| DELETE | `/api/v1/license` | Remove the license, revert to free |
