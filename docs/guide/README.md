...
How to use the whole system: run Servienta, drive it from your tests, inject
...
The **application under test is out of scope**: Servienta never reaches into
...
| --- | --- | --- |
| **Servienta** | The delivered container: receivers, file server, control API, embedded console | Your infrastructure — one Docker container, offline-capable |
| **Console** | Management UI: view the stand, apply a license, reset | Built into Servienta container, on :5000 (D20) |
...
| Page | Covers |
| --- | --- |
| [Quickstart](quickstart.md) | Run Servienta, record and read your first message |
| [API reference](api.md) | Every control endpoint |
...
- **Instance** — one bring-up of Servienta (one `docker run`). Mutations
...
- **Source claim** — a run declares the source addresses whose traffic belongs
  to it; Servienta attributes each message to a run by source. This is what
